...
values	('Villa Beach Front', 25000, 1000000, 10, 'vip', 'Có hồ bơi', 500, 4, 3, 1),
		('House Princess 01', 14000, 500000, 7, 'vip', 'Có thêm bếp nướng',null,3, 2, 2),
        ('Room Twin 01', 5000, 100000, 2, 'normal', 'Có tivi',null, null, 4, 3),
        ('Villa No Beach Front', 22000, 900000, 8, 'normal', 'Có hồ bơi', 300, 3, 3, 1),
...
values	('2020-12-08', '2020-12-08', 0, 3, 1, 3),
        ...
        ('2021-06-17', '2021-06-18', 1500000, 3, 4, 1),
...
-- câu 2 	Hiển thị thông tin của tất cả nhân viên có trong hệ thống có tên bắt đầu là một
...
ifnull(ifnull(sum(hdct.so_luong * dvdk.gia),0) + phidichvu.gia_dich_vu,0) as tong_tien
from khach_hang kh       
...
left join (
	select kh.ma_khach_hang, kh.ho_ten, sum(dv.chi_phi_thue) as gia_dich_vu
    from khach_hang kh
...
on kh.ma_khach_hang = phidichvu.ma_khach_hang
group by kh.ma_khach_hang;
...
select * 
from hop_dong hd
where year(hd.ngay_lam_hop_dong) = 2021 and dv.ma_dich_vu = hd.ma_dich_vu
)
group by dv.ma_dich_vu;
...
-- cach: không hiển thị họ tên trùng nhau
select ho_ten
from khach_hang
group by ho_ten
having count(ho_ten)=1;

-- cach : ho ten trung nhau chỉ hiển thị 1 người
...
-- câu 9 9.	Thực hiện thống kê doanh thu theo tháng, 
...
-- câu 11.	Hiển thị thông tin các dịch vụ đi kèm đã được sử dụng bởi những khách hàng có ten_loai_khach là “Diamond” 
...
where hd.ngay_lam_hop_dong between '2020-09-01' and '2020-12-31'
and not exists
(
select * 
from hop_dong hd
where year(hd.ngay_lam_hop_dong) between '2021-01-01' and '2021-06-30' and dv.ma_dich_vu = hd.ma_dich_vu
)
group by hd.ma_hop_dong;
...
where year(hd.ngay_lam_hop_dong) between 2020 and 2021 -- (hoặc year(hd.ngay_lam_hop_dong) in (2020,2021))
...
and kh.ma_loai_khach in (
select temp.ma_khach_hang from 
(
select kh.ma_khach_hang, ifnull(ifnull(sum(hdct.so_luong * dvdk.gia),0) + phithue.gia_dich_vu,0) as tong_tien
from khach_hang kh       
left join loai_khach lk on kh.ma_loai_khach = lk.ma_loai_khach
left join hop_dong hd on kh.ma_khach_hang = hd.ma_khach_hang
left join dich_vu dv on hd.ma_dich_vu = dv.ma_dich_vu
...
on kh.ma_khach_hang = phithue.ma_khach_hang
...
group by hdct.ma_dich_vu_di_kem)temp);
...
select nv.ma_nhan_vien as 'id(ma_nhan_vien, ma_khach_hang)' , nv.ho_ten, nv.email, nv.so_dien_thoai, nv.ngay_sinh, nv.dia_chi from nhan_vien nv
union
